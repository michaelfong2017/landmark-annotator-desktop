...
void AnnotateTab::reloadCurrentImage(QImage colorImageLeft, cv::Mat depthMapToColorImage) {
	...
	if (depthMapToColorImage.cols == 800 && depthMapToColorImage.rows == 1080) {
		...
		query3DOffsetX = 560;
		camera::CameraManager::getInstance().setCameraForIntrinsics(camera::Model::KINECT);
	}
		...
		query3DOffsetX = 373;
		camera::CameraManager::getInstance().setCameraForIntrinsics(camera::Model::REALSENSE);
	}
	else {
		qCritical() << "Cropped image must have dimensions either 800x1080 or 534x720, but now is not.";
	}

	camera::CameraManager::getInstance().getCameraForIntrinsics()->readIntrinsicsFromFile("intrinsics_realsense_color.txt");
...
void AnnotateTab::resizeAndDrawAnnotations() {
	...
	for (auto it : this->annotationsOnRight) {
		...
		QVector3D vector3D = camera::CameraManager::getInstance().getCameraForIntrinsics()->query3DPoint(x + this->query3DOffsetX, y, this->recalculatedFullResolutionDepthImage);
